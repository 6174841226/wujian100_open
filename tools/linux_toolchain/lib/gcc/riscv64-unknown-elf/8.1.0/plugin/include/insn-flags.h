...
#define HAVE_extvdi (TARGET_XCKI_EXT)
#define HAVE_extzvdi (TARGET_XCKI_EXT)
...
#define HAVE_maddhisi4 (TARGET_MUL && TARGET_64BIT && TARGET_XCKI && riscv_insn_mula)
#define HAVE_msubhisi4 (TARGET_MUL && TARGET_64BIT && TARGET_XCKI && riscv_insn_mula)
...
#define HAVE_bswapsi2 (TARGET_64BIT && TARGET_XCKI && riscv_insn_revw)
#define HAVE_bswapdi2 (TARGET_64BIT && TARGET_XCKI && riscv_insn_rev)
...
#define HAVE_store_pairdi (TARGET_XCKI_LDD \
   && reload_completed \
   && REGNO (operands[3]) == REGNO (operands[1]) + 1 \
   && rtx_equal_p (XEXP (operands[2], 0), \
      plus_constant (Pmode, XEXP (operands[0], 0), GET_MODE_SIZE (DImode))))
#define HAVE_load_pairdi (TARGET_XCKI_LDD \
   && reload_completed \
...
#define HAVE_rotrsi3 (TARGET_64BIT && TARGET_XCKI && riscv_insn_srriw)
#define HAVE_rotrdi3 (TARGET_64BIT && TARGET_XCKI && riscv_insn_srri)
#define HAVE_rotrsi_signext (TARGET_64BIT && TARGET_XCKI && riscv_insn_srriw \
   && ((INTVAL (operands[2]) + INTVAL (operands[3])) == 32))
#define HAVE_cmovzdi (TARGET_64BIT && TARGET_XCKI && riscv_insn_condmv)
#define HAVE_cmovndi (TARGET_64BIT && TARGET_XCKI && riscv_insn_condmv)
...
#define HAVE_movsf 1
#define HAVE_movdf 1
#define HAVE_movmemsi (!(!STRICT_ALIGNMENT && TARGET_64BIT && TARGET_XCKI))
#define HAVE_movmemdi (!STRICT_ALIGNMENT && TARGET_64BIT && TARGET_XCKI)
#define HAVE_clear_cache 1
#define HAVE_movdicc (TARGET_64BIT && TARGET_XCKI && !optimize_size && riscv_insn_condmv)
...
extern rtx        gen_rotrsi_signext            (rtx, rtx, rtx, rtx);
extern rtx        gen_cmovzdi                   (rtx, rtx, rtx, rtx);
extern rtx        gen_cmovndi                   (rtx, rtx, rtx, rtx);
